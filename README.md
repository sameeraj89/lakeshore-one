...
| `intranet/` | Intranet | Staff portal for clinicians, nurses and management, personalised by role: global search across everything; **what's new** with must-read acknowledgement; a controlled **policy library** (48 documents with version, effective/review dates, owner and NABH chapter, acknowledgement tracking); **clinical reference** — emergency codes, today's on-call roster, critical/panic values, antimicrobial stewardship list, rapid-response bundles; directory with key extensions; task-based services ("I need to…"); mandatory training compliance, benefits & wellbeing, events calendar |
...
| `ot-schedule/` | OT Schedule | Operation theatre planning — book cases with table-clash checks (20-min turnover protected), six-theatre day timeline 07:00–21:00, case list with statuses, utilisation insights. Linked to the live OT stage board in the main app: confirming a booking puts the case (procedure & surgeon only — patient identity stays on the planning board) on the stage board; wheel-in/done/cancel there reflect back, and delays/cancellations here follow the case there |
