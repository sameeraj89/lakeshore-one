...
const MIGRATIONS = [
  ...
  () => addColumn('users', 'email TEXT'),                            /* v3: Google SSO account linking */
  () => addColumn('ot_bookings', 'case_id TEXT'),                    /* v4: booking -> live OT stage-board case */
];
function migrate(){
...
function opChangeKind(k){
  ...
  if (k === 'user_add' || k === 'user_email' || k === 'user_toggle' || k === 'pin_reset') return 'user';
  if (k === 'praise' || k === 'praise_del') return 'praise';
  if (k.startsWith('pi_')) return 'patient_input';
  if (k.startsWith('usg_')) return 'usg';
  if (k.startsWith('otb_')) return 'ot_booking';
  return 'ticket';
}
...
function buildState(u){
  ...
  if (!isLimited(role)){
    ...
    /* batched OT milestones (one query for all cases) instead of one per case */
    ...
    cases = rawCases.map(c => ({ id:c.id, suite:c.suite, date:c.case_date, planned:c.planned, durMin:c.dur_min,
  ...
  try{
    ...
    if (p === '/api/guest' && req.method === 'POST'){
      ...
      if (as === 'nurse'){
        ...
        const nu = db.prepare("SELECT tok_epoch FROM users WHERE emp_id='LH-NURSE'").get();
        return json(res, 200, { token:sign('LH-NURSE', nu ? nu.tok_epoch : 0), empId:'LH-NURSE' });
      }
      ...
      return json(res, 200, { token:sign(eid, 0), empId:eid });   /* fresh row, epoch 0 */
    }
    ...
    if (p === '/api/google-login' && req.method === 'POST'){
      ...
      return json(res, 200, { token:sign(u.emp_id, u.tok_epoch), empId:u.emp_id });
    }
    ...
    if (p.startsWith('/api/')){
      ...
      if (p === '/api/op' && req.method === 'POST'){
        ...
        if (result.ok){ broadcast(opChangeKind(op.kind)); whatsapp.onOp(op, u); }
